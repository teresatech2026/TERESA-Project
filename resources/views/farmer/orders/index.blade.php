                ...
                @else
                    <div class="space-y-4">
                        @foreach ($orders as $order)
                            <a href="{{ route('farmer.orders.show', $order) }}" class="block border rounded-lg p-4 hover:shadow-md transition">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <p class="font-semibold">Order #{{ $order->id }} — {{ $order->buyer->full_name }}</p>
